...
namespace homework10 {
      ...
      pending = new Queue<string>();
            ...
            while ( urls.Count < MaxPage )
            {
                ...
                try
                {
                    ...
                    threadInfo.html = DownLoad(url);
                    threadInfo.url = url;
                    urls[url] = true;
                    PageDownloaded(this, url, "success");
                    int index = tasks.Count;
                    Task task = Task.Run(() => Parse(threadInfo));
                    tasks.Add(task);
                    //Parse(threadInfo);//解析,并加入新的链接
                }
                catch (Exception ex)
                {
    ...
    private void Parse(object a) {
            ThreadInfo info = a as ThreadInfo;
            var matches = new Regex(urlDetectRegex).Matches(info.html);
      foreach (Match match in matches) {
        string linkUrl = match.Groups["url"].Value;
        if (linkUrl == null || linkUrl == "") continue;
        linkUrl = FixUrl(linkUrl, info.url);//转绝对路径
